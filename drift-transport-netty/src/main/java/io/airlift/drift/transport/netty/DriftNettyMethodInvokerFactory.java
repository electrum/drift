        ...
        implements MethodInvokerFactory<I>, Closeable
{
    ...
    public MethodInvoker createMethodInvoker(AddressSelector addressSelector, I clientIdentity)
    {
        ...
        switch (clientConfig.getProtocol()) {
            case BINARY:
                protocolFactory = new TBinaryProtocol.Factory(false, true, -1, clientConfig.getMaxFrameSize());
                break;
            case COMPACT:
                // Header transport uses the FB fork of the compact protocol
                if (clientConfig.getTransport() == Transport.HEADER) {
                    protocolFactory = new TFacebookCompactProtocol.Factory(clientConfig.getMaxFrameSize());
                }
                else {
                    protocolFactory = new TCompactProtocol.Factory(-1, clientConfig.getMaxFrameSize());
                }
                break;
            default:
        ...
        if (clientConfig.isSslEnabled()) {
            try {
                SslContextBuilder sslContextBuilder = SslContextBuilder.forClient()
                        ...
                        .sessionTimeout(clientConfig.getSessionTimeoutSeconds());
                if (clientConfig.getCiphers() != null) {
